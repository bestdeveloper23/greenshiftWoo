...
class GSPBWOO_WC_Group_Attributes {
	...
	public function register_attribute_group() {
		$labels = array(
			'name'                => __( 'Attribute Groups', 'greenshiftwoo' ),
			'singular_name'       => __( 'Attribute Group', 'greenshiftwoo' ),
			'add_new'             => __( 'Add New Attribute Group', 'greenshiftwoo' ),
			'add_new_item'        => __( 'Add New Attribute Group', 'greenshiftwoo' ),
			'edit_item'           => __( 'Edit Attribute Group', 'greenshiftwoo' ),
			'new_item'            => __( 'New Attribute Group', 'greenshiftwoo' ),
			'view_item'           => __( 'View Attribute Group', 'greenshiftwoo' ),
			'search_items'        => __( 'Search Attribute Groups', 'greenshiftwoo' ),
			'not_found'           => __( 'No Attribute Groups found', 'greenshiftwoo' ),
			'not_found_in_trash'  => __( 'No Attribute Groups found in Trash', 'greenshiftwoo' ),
			'parent_item_colon'   => __( 'Parent Attribute Group:', 'greenshiftwoo' ),
			'menu_name'           => __( 'Attribute Groups', 'greenshiftwoo' ),
		);
	...
	public function register_attribute_group_taxonomy() {
        ...
        $singular = __('Attribute Group Category', 'greenshiftwoo');
        $plural = __('Attribute Group Categories', 'greenshiftwoo');

        $labels = array(
            'name' => $plural,
            'singular_name' => $singular,
            'search_items' => sprintf(__('Search %s', 'greenshiftwoo'), $plural),
            'all_items' => __('All Categories', 'greenshiftwoo'),
            'parent_item' => sprintf(__('Parent %s', 'greenshiftwoo'), $singular),
            'parent_item_colon' => sprintf(__('Parent %s:', 'greenshiftwoo'), $singular),
            'edit_item' => sprintf(__('Edit %s', 'greenshiftwoo'), $singular),
            'update_item' => sprintf(__('Update %s', 'greenshiftwoo'), $singular),
            'add_new_item' => sprintf(__('Add New %s', 'greenshiftwoo'), $singular),
            'new_item_name' => sprintf(__('New %s Name', 'greenshiftwoo'), $singular),
            'menu_name' => $plural,
        );
    ...
    public function enqueue_styles() {
		...
		wp_enqueue_style( 'greenshiftwoo-select2', GREENSHIFTWOO_DIR_URL .'/assets/css/select2.min.css', array(), '3.5.4', 'all' );
        wp_enqueue_style( 'greenshiftwoo-select2-sortable', GREENSHIFTWOO_DIR_URL .'/assets/css/select2.sortable.min.css', array(), $this->version, 'all' );
    }

	/*  */
    public function enqueue_scripts() {
		wp_enqueue_script('woo-group-attributes-admin', GREENSHIFTWOO_DIR_URL .'/assets/js/woo-group-attributes-admin.js', array('jquery'), $this->version, true);
        ...
        wp_enqueue_script( 'greenshiftwoo-select2', GREENSHIFTWOO_DIR_URL .'/assets/js/select2.min.js', array('jquery'), '3.5.4', true );
        wp_enqueue_script( 'greenshiftwoo-select2-sortable', GREENSHIFTWOO_DIR_URL .'/assets/js/select2.sortable.min.js', array('jquery'), $this->version, true );
        wp_enqueue_script( 'greenshiftwoo-html5-sortable', GREENSHIFTWOO_DIR_URL .'/assets/js/html.sortable.min.js', array('jquery'), $this->version, true );
    }
	...
	public function columns_head( $columns ) {
		...
		foreach( $columns as $column => $name ){
			$output[$column] = $name;
			if( $column === 'title' ){
				$output['attributes'] = esc_html__( 'Attributes', 'greenshiftwoo' );
			}
		}
    ...
    public function add_custom_metaboxes() {
        add_meta_box('woocommerce_group_attributes_metabox', esc_html__( 'Attributes', 'greenshiftwoo' ), array($this, 'attributes'), 'attribute_group', 'normal', 'high');
    }
    ...
    public function attributes() {
		...
		/* echo '<label for="'. $prefix .'attributes">'. esc_html__( 'Attributes', 'greenshiftwoo' ) .':</label><br/>'; */
        $order = "";
    ...
    public function save_custom_metaboxes( $post_id ) {
    	...
    	$multiple_attributes_in_groups = apply_filters( 'gspbwoo_multiple_attributes_in_groups', false );
    ...
    public function show_attribute_group_toolbar() {
		...
		?>
		<div class="toolbar">
			<h3 class="attribute_groups_tiltle"><?php esc_html_e( 'Product Attribute Groups', 'greenshiftwoo' ); ?></h3>
			<?php
			if( apply_filters( 'gspbwoo_enable_attribute_group_categories', true ) ):
				$attribute_group_categories = get_terms( array(
					'taxonomy' => 'attribute_group_categories',
					'hide_empty' => true,
				) );
			?>
			<button type="button" id="load_attribute_group_category" class="button button-primary" style="float: right;margin: 0 0 0 6px;"><?php esc_html_e( 'Load', 'greenshiftwoo' ); ?></button>
			<select id="woocommerce_attribute_group_categories" name="woocommerce_attribute_group_categories" class="woocommerce_attribute_group_categories" style="float: right;margin: 0 0 0 6px;">
				<option value=""><?php esc_html_e( 'Groups Categories', 'greenshiftwoo' ); ?></option>
				<?php 
			...
			<?php endif; ?>
			<button type="button" id="load_attribute_group" class="button button-primary" style="float: right;margin: 0 0 0 6px;"><?php esc_html_e( 'Load', 'greenshiftwoo' ); ?></button>
			<select id="woocommerce_attribute_groups" name="woocommerce_attribute_groups" class="woocommerce_attribute_groups" style="float: right;margin: 0 0 0 6px;">
				<option value=""><?php esc_html_e( 'Current Groups', 'greenshiftwoo' ); ?></option>
				<?php 
				foreach ($attribute_groups as $attribute_group) {
			...
			<a href="<?php echo admin_url( 'edit.php?post_type=attribute_group' ); ?>" class="button" onclick="return confirm('<?php esc_html_e( 'Are you sure you want to navigate away.', 'greenshiftwoo' ); ?>');"><?php esc_html_e( 'Manage Groups', 'greenshiftwoo' ); ?></a>
		</div>
		<?php
    }
	...
	*/
	public function modify_attribute_template( $located, $template_name) {
		if('single-product/product-attributes.php' === $template_name){
			...
			return apply_filters( 'gspbwoo_woo_group_attributes_layout', GREENSHIFTWOO_DIR_PATH .'/inc/templates/woo-group-attributes-public.php' );
		}
		return $located;
	}
}