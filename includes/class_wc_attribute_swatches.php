...
class GSPBWOO_Attribute_Swatches{

    public $meta_color = 'gspbwoo_swatch_color';
    public $meta_image = 'gspbwoo_swatch_image';
    public $meta_text = 'gspbwoo_swatch_text';
    public $meta_tooltip = 'gspbwoo_swatch_tooltip';
    public $option_name = 'gspbwoo_wc_swatches';
    public $meta_image_tag = 'gspbwoo_swatch_image_tag';
    public $meta_text_tag = 'gspbwoo_swatch_text_tag';
    ...
    function attribute_swatch_column( $type ){
        $type = array(
            'select' => esc_html__( 'Select', 'greenshiftwoo' ),
            'color' => esc_html__( 'Color', 'greenshiftwoo' ),
            'image' => esc_html__( 'Image', 'greenshiftwoo' ),
            'text' => esc_html__( 'Text tag', 'greenshiftwoo' ),
            'image_tag' => esc_html__( 'Image tag', 'greenshiftwoo' )
        );
        return $type;
    }
    ...
    function add_color_field(){
        ...
        <div class="form-field term-color-wrap">
            <label for="term-color"><?php esc_html_e( 'Swatch Color', 'greenshiftwoo' ); ?></label>
            <input name="swatch_color" id="term-color" type="text" value="" size="40" />
        </div>
        <div class="form-field term-text-wrap">
            <label for="term-tooltip"><?php esc_html_e( 'Swatch Tooltip', 'greenshiftwoo' ); ?></label>
            <input name="swatch_tooltip" id="term-tooltip" type="text" value="" size="40" />
        </div>
        <?php
    }
    ...
    function edit_color_field( $tag ){
        ...
        $tooltip = get_term_meta( $tag->term_id, $this->meta_tooltip, true );
        wp_enqueue_script( 'wp-color-picker' );
        wp_enqueue_style( 'wp-color-picker' );
        wp_add_inline_script( 'wp-color-picker', 'jQuery(document).ready(function($){$("#term-color").wpColorPicker();});');
        ?>
        <tr class="form-field term-color-wrap">
            <th scope="row"><label for="term-color"><?php esc_html_e( 'Swatch Color', 'greenshiftwoo' ); ?></label></th>
        ...
        <tr class="form-field term-text-wrap">
            <th scope="row"><label for="term-tooltip"><?php esc_html_e( 'Swatch Tooltip', 'greenshiftwoo' ); ?></label></th>
            <td><input name="swatch_tooltip" id="term-tooltip" type="text" value="<?php echo esc_html($tooltip); ?>" size="40" /></td>
        </tr>
        <?php
    }

    /* Adds 'Image Tag' fields to Attribute term creating form */
    function add_image_tag_field(){
        wp_enqueue_media();
        ?>
        <div class="form-field term-image-wrap">
            <label><?php esc_html_e( 'Swatch Image', 'greenshiftwoo' ); ?></label>
            ...
            <div style="line-height: 60px;">
                <input type="hidden" id="term-image" name="swatch_image_tag" />
                <button type="button" class="upload_image_button button"><?php esc_html_e( 'Upload/Add image', 'greenshiftwoo' ); ?></button>
                <button type="button" class="remove_image_button button"><?php esc_html_e( 'Remove image', 'greenshiftwoo' ); ?></button>
            </div>
            <script type="text/javascript">
                ...
                jQuery( document ).on( 'click', '.upload_image_button', function( event ) {
                    ...
                    file_frame = wp.media.frames.downloadable_file = wp.media({
                        title: '<?php esc_html_e( 'Choose an image', 'greenshiftwoo' ); ?>',
                        button: {
                            text: '<?php esc_html_e( 'Use image', 'greenshiftwoo' ); ?>'
                        },
                        multiple: false
                    });
        ...
        <div class="form-field term-text-wrap">
            <label for="term-text"><?php esc_html_e( 'Swatch Text', 'greenshiftwoo' ); ?></label>
            <input name="swatch_text_tag" id="term-text" type="text" value="" size="40" />
        </div>
        <div class="form-field term-text-wrap">
            <label for="term-tooltip"><?php esc_html_e( 'Swatch Tooltip', 'greenshiftwoo' ); ?></label>
            <input name="swatch_tooltip" id="term-tooltip" type="text" value="" size="40" />
    ...
    /* Adds 'Image Tag' fields to Attribute term editing form */
    function edit_image_tag_field( $tag ){
        $text = get_term_meta( $tag->term_id, $this->meta_text_tag, true );
        $image_id = absint( get_term_meta( $tag->term_id, $this->meta_image_tag, true ) );
        $image = ( $image_id ) ? wp_get_attachment_thumb_url( $image_id ) : wc_placeholder_img_src();
        $tooltip = get_term_meta( $tag->term_id, $this->meta_tooltip, true );
        wp_enqueue_media();
        ?>
        <tr class="form-field term-image-wrap">
            <th scope="row" valign="top"><label><?php esc_html_e( 'Swatch Image', 'greenshiftwoo' ); ?></label></th>
            <td>
                <div id="term-image-preview" style="float: left; margin-right: 10px;"><img src="<?php echo esc_url( $image ); ?>" width="60px" height="60px" /></div>
                <div style="line-height: 60px;">
                    <input type="hidden" id="term-image" name="swatch_image_tag" value="<?php echo esc_html($image_id); ?>" />
                    <button type="button" class="upload_image_button button"><?php esc_html_e( 'Upload/Add image', 'greenshiftwoo' ); ?></button>
                    <button type="button" class="remove_image_button button"><?php esc_html_e( 'Remove image', 'greenshiftwoo' ); ?></button>
                </div>
                <script type="text/javascript">
        ...
        <tr class="form-field term-text-wrap">
            <th scope="row"><label for="term-text"><?php esc_html_e( 'Swatch Text', 'greenshiftwoo' ); ?></label></th>
            <td><input name="swatch_text_tag" id="term-text" type="text" value="<?php echo esc_html($text); ?>" size="40" /></td>
        </tr>
        <tr class="form-field term-text-wrap">
            <th scope="row"><label for="term-tooltip"><?php esc_html_e( 'Swatch Tooltip', 'greenshiftwoo' ); ?></label></th>
            <td><input name="swatch_tooltip" id="term-tooltip" type="text" value="<?php echo esc_html($tooltip); ?>" size="40" /></td>
    ...
    function save_term_fields( $term_id, $tt_id = '', $taxonomy = '' ){
        ...
        if ( isset( $_POST['swatch_tooltip'] ) ) {
            update_woocommerce_term_meta( $term_id, $this->meta_tooltip, sanitize_text_field( $_POST['swatch_tooltip'] ) );
        }
        if ( isset( $_POST['swatch_image_tag'] ) ) {
            update_woocommerce_term_meta( $term_id, $this->meta_image_tag, absint( $_POST['swatch_image_tag'] ) );
        }
        if ( isset( $_POST['swatch_text_tag'] ) ) {
            update_woocommerce_term_meta( $term_id, $this->meta_text_tag, sanitize_text_field( $_POST['swatch_text_tag'] ) );
        }
    }
        ...
        if( is_object( $attribute ) ) :
            switch( $attribute->type ){
                ...
                case 'text':
                    ...
                    break;
                case 'image_tag':
                    $meta_name = $this->meta_image_tag;
                    break;
                default:
                    $meta_name = '';
            }
            $meta_tooltip = $this->meta_tooltip;
            if( !empty( $meta_name ) ) {
                if(!empty($meta_tooltip)){
                    delete_woocommerce_term_meta( $term_id, $meta_tooltip );
                }
    ...
    public function swatch_columns( $columns ) {
        ...
        $new_columns['swatch'] = esc_html__( 'Swatch', 'greenshiftwoo' );
        $columns = array_merge( $new_columns, $columns );
        $columns['handle'] = '';
        return $columns;
    }

    /* Adds Swatch Image Tag column to term table list */
    function swatch_image_tag_column( $columns, $column, $term_id ) {
        if ( 'swatch' === $column ) {
            $thumbnail_id = get_term_meta( $term_id, $this->meta_image_tag, true );
            $text = get_term_meta( $term_id, $this->meta_text_tag, true );
            if ( $thumbnail_id ) {
            ...
            $columns .= '<img src="' . esc_url( $image ) . '" alt="' . esc_attr__( 'Swatch', 'greenshiftwoo' ) . '" class="wp-post-image" height="48" width="48" />';
            $columns .= '<div>' . esc_html( $text ) . '</div>';
        }
        if ( 'handle' === $column ) {
    ...
    function add_attribute_to_product( $attribute_taxonomy, $i, $attribute ){
        ...
        if ( 'select' !== $attribute_taxonomy->attribute_type ) {
            ?>
            <select multiple="multiple" data-placeholder="<?php esc_attr_e( 'Select terms', 'greenshiftwoo' ); ?>" class="multiselect attribute_values wc-enhanced-select" name="attribute_values[<?php echo esc_attr( $i ); ?>][]">
                <?php
                $args = array(
                    'orderby'    => 'name',
                    'hide_empty' => 0,
            ...
            <button class="button plus select_all_attributes"><?php esc_html_e( 'Select all', 'greenshiftwoo' ); ?></button>
            <button class="button minus select_no_attributes"><?php esc_html_e( 'Select none', 'greenshiftwoo' ); ?></button>
            <button class="button fr plus add_new_attribute"><?php esc_html_e( 'Add new', 'greenshiftwoo' ); ?></button>
            <?php
        }
    }
}

new GSPBWOO_Attribute_Swatches();