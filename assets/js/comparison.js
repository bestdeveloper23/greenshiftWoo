...
function getLinksToCompareOnPage() {
    const elLinkToCompareOnPage = document.querySelectorAll('.gs-comparison-icon-link-to-compare')
    let links = ''
...
let gscomparecounter = document.getElementsByClassName('gscompareplus');
...
const requestCompareUpdate = new XMLHttpRequest();
requestCompareUpdate.open('POST', gscomparevars.ajax_url, true);
requestCompareUpdate.setRequestHeader('Content-Type', 'application/x-www-form-urlencoded; charset=UTF-8');
requestCompareUpdate.responseType = 'json';
requestCompareUpdate.onload = function () {
    if (this.status >= 200 && this.status < 400) {
        let responseobj = this.response;
        if (responseobj.comparisonids) {
            let comparisonids = responseobj.comparisonids.split(',');
            if (comparisonids.length != 0) {
                let favListed = document.querySelectorAll(".gscompareplus");
...
requestCompareUpdate.onerror = function () {
    // Connection error
};
requestCompareUpdate.send('action=gscomparerecount&comparenonce=' + compare_nonce);