...
define('GREENSHIFTWOO_PLUGIN_VER', '1.6.1');
...
if (!function_exists('greenShiftWoo_register_init')) {
    function greenShiftWoo_register_init()
    {
        ...
        wp_register_script(
            ...
            GREENSHIFTWOO_DIR_URL . 'assets/js/comparison.min.js',
            array(),
            '1.1',
            true
        );
        ...
        wp_register_script(
            ...
            GREENSHIFTWOO_DIR_URL . 'assets/js/tablechart.min.js',
            array(),
            '1.0',
            true
        );
        ...
        wp_register_script(
            'gspbcompare',
            GREENSHIFTWOO_DIR_URL . 'assets/js/comparechart.min.js',
        ...
        if ('Rehub theme' != $theme->name && 'Rehub theme' != $theme->parent_theme) {
            wp_register_script(
                'gsswatches',
                GREENSHIFTWOO_DIR_URL . 'assets/js/wooswatch.min.js',
                array('jquery'),
                '1.4.2',
                true
            );
        ...
        wp_register_script(
            'gsvariationajax',
            GREENSHIFTWOO_DIR_URL . 'assets/js/variationajax.js',
            array('jquery'),
            '1.1',
            true
        );
...
if (!function_exists('greenShiftWoo_editor_assets')) {
    function greenShiftWoo_editor_assets()
    {
        ...
        wp_set_script_translations('greenShiftWoo-block-js', 'greenshiftwoo');
...
function gspbwoo_get_product_parts_callback(WP_REST_Request $request)
{
    ...
    switch ($part) {
        ...
        case 'product_images':
            ...
            $result = empty($featured_image) ? [wc_placeholder_img_src('woocommerce_single')] : gspbwoo_get_all_images_url_of_product($_product);
            break;
        default:
            break;
    }
...
function gspbwoo_get_all_images_url_of_product($_product)
{
    $featured_image = $_product->get_image_id();
    $image_ids = $_product->get_gallery_image_ids();

    $result = [wp_get_attachment_image_url($featured_image, 'woocommerce_single')];

    foreach ($image_ids as $image_id) {
        $result[] = wp_get_attachment_image_url($image_id, 'woocommerce_single');
    }
...
if (!function_exists('greenShiftWoo_block_script_assets')) {
    function greenShiftWoo_block_script_assets($html, $block)
    {
        ...
        if (!is_admin()) {
            ...
            if ($block['blockName'] == 'greenshift-blocks/product-quick-view') {
            ...
            if ($block['blockName'] == 'greenshift-blocks/product-comparison' && !empty($block['attrs']['type']) && $block['attrs']['type'] == 'table') {
                add_action('wp_footer', 'show_search_form');
            }

            if ($block['blockName'] == 'greenshift-blocks/woo-cart-button') {
                wp_enqueue_script('gspbwoocartbutton');
            }

            if ($block['blockName'] == 'greenshift-blocks/product-rating') {
                if(!empty($block['attrs']['innerPage'])){
                    wp_enqueue_script('gspbreviewlink');
                }
            }

            if ($block['blockName'] == 'greenshift-blocks/product-gallery') {
            ...
            if ($block['blockName'] == 'greenshift-blocks/product-tabs') {
...
function greenshiftwoo_add_to_cart_fragment_amount( $fragments ) {
    ...
    ?>
        <?php echo '
		    <span class="gspb_woocartmenu-amount">' . WC()->cart->get_total() . '</span>';
        ?>
    <?php
...
add_filter( 'load_script_translation_file', 'greenshiftwoo_custom_block_fix_translation_location', 10, 3 );
function greenshiftwoo_custom_block_fix_translation_location( string $file, string $handle, string $domain ): string {
	if ( strpos( $handle, 'greenShiftWoo-block-js' ) !== false && 'greenshiftwoo' === $domain ) {
